...
class Visualization(object):
    ...
    def tIoU(self, start, end, pred_start, pred_end):
        ...
        # Segment union.
        segments_union = (pred_end - pred_start)+ (end - start) - segments_intersection
    ...
    def run(self, indexes, pred_start, pred_end, start, end, lengths,
                ...
                attentionNodeQueryHO, attentionNodeQueryVH, attentionNodeQueryVO):
        l = loss.detach().item()
        self.loss.append(l)
        ...
        for j, indx in enumerate(indexes):
            ret[int(indx)] = {"iou": round(iou[j], 4),
                        "p_start": round(startings[j], 4),
                        "p_end": round(endings[j], 4),
                        "attQHO": attentionNodeQueryHO[j].cpu().detach().numpy().tolist(),
                        "attQVH": attentionNodeQueryVH[j].cpu().detach().numpy().tolist(),
                        "attQVO": attentionNodeQueryVO[j].cpu().detach().numpy().tolist()}
